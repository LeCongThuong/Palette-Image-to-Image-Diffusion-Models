...
class WoodblockDataset(data.Dataset):
    def __init__(self, data_root, mode="train", image_size=[512, 512]):
        self.data_root = data_root
        if mode != "valid":
            self.print_path_list = sorted(list(Path(os.path.join(self.data_root, mode, "print_512")).glob("*.png")), key=os.path.basename)[1200:]
            self.depth_path_list = sorted(list(Path(os.path.join(self.data_root, mode, "np_depth_512")).glob("*.npy")), key=os.path.basename)[1200:]
        else:
